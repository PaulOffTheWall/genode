/**
 * $Id: task.h 815 2014-12-15 13:40:52Z klugeflo $
 * @file task.h
...
namespace tmssim {
    ...
    public ICloneable<Task>
  {
     ...
     */
    Task(unsigned int __id, int __executionTime, int __relDeadline,
    ...
    int getExecutionTime(void) const;

    /// @return the critical time (=deadline) of the task
    int getRelativeDeadline(void) const;
    ...
    */
    void completeJob(Job* job, int now);
  ...
  protected:
     ...
     */
    virtual int startHook(int now) = 0;
     ...
     */
    virtual Job* spawnHook(int now) = 0;
     ...
     * available
     */
    virtual int getNextActivationOffset(int now) = 0;
    /**
      ...
      @param now current simulated time
    */
    virtual bool completionHook(Job *job, int now) = 0;
    /**
      This method is called by #tmssim::Task::cancelJob.
      @param job the cancelled job
      @return true, if the job was successfully cleaned up
    ...
    */
     ...
     * subclass.
     */
    virtual char getShortId(void) const = 0;
    ...
    const int& executionTime;  ///< fast ro access to execution time
    const int& absDeadline; ///< fast ro access to critical time
    const int& priority; ///< fast ro access
    /// @}
  ...
  private:
    ...
    int _executionTime; ///< execution time
    int _relDeadline; ///< relative critical time
    ...
    int _priority; ///< task (static) priority
    int _nextActivation; ///< when will the next job be generated?
